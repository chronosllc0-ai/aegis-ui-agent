...
## 2026-04-11 — Main branch review remediation (6 findings)

### What changed
- `backend/pydantic_adk_runner.py`
  - Added guardrail for unbounded execution:
    - configurable `max_tool_calls` (default 100) enforced inside `run_tool(...)`.
  - Added model-call timeout protection:
    - wraps `agent.run(...)` in `asyncio.wait_for(..., timeout=model_timeout_seconds)` (default 120s) in both cancel-event and non-cancel paths.
- `main.py`
  - Adjusted instruction normalization to explicit `None` checks so fallback selection is deterministic and not based on generic falsy coercion:
    - `instruction = str(raw_instruction if raw_instruction is not None else raw_prompt if raw_prompt is not None else "").strip()`
- `frontend/src/App.tsx`
  - Removed reconnect-retry dependency churn by moving sub-agent lookups to `subAgentsRef` and removing `subAgents`/`selectedTaskId` from `handleSend` callback deps.
  - Kept retry effect dependency-complete (`handleSend` included) without suppressions.
- `frontend/src/components/ChatPanel.tsx`
  - Restored stop button visibility rule to only show when actively working **and** there is no draft input (`isWorking && !canSend`) to avoid send/stop overlap.
- Removed dead file `frontend/src/components/InputBar.tsx` (unused by the active UI path).

### Why
- Addresses all reported review items:
  - unbounded tool loop risk,
  - missing model timeout,
  - instruction parsing warning,
  - reconnect retry callback dependency churn,
  - stop/send overlap UX,
  - dead InputBar component nitpick.
...
### Notes
- Existing Vite chunk-size warning (>500 kB) remains unrelated to these fixes.
