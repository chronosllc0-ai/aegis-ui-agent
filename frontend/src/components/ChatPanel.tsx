...
export interface ChatPanelProps {
  ...
  subAgentMentions?: Array<{ handle: string; label: string }>
...
export function ChatPanel({
  ...
  const mentionOptions = useMemo(() => {
    ...
    return subAgentMentions
      .filter((item) => item.handle.toLowerCase().includes(mentionQuery) || item.label.toLowerCase().includes(mentionQuery))
      .slice(0, 6)
  }, [mentionQuery, subAgentMentions])

  const handleMentionPick = (handle: string) => {
    setInput((prev) => prev.replace(/@([a-zA-Z0-9._-]*)$/, `@${handle} `))
          ...
          <div className='mb-2 rounded-xl border border-[#2a2a2a] bg-[#141414] p-1.5'>
            {mentionOptions.map((item) => (
              <button
                key={item.handle}
                type='button'
                onClick={() => handleMentionPick(item.handle)}
              ...
              >
                ...
                <span className='mx-2 truncate text-zinc-500'>{item.label}</span>
