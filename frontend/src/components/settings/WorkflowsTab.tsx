        ...
        {workflows.map((workflow) => (
          <article key={workflow.id} className='rounded-2xl border border-[#2a2a2a] bg-[#111] p-4'>
            ...
            <div className='mt-3 flex flex-wrap gap-2 text-xs'>
              <button type='button' onClick={() => onRun(workflow.instruction)} className='inline-flex items-center gap-1 rounded-md border border-[#2a2a2a] px-2 py-1'>{Icons.play({ className: 'h-3.5 w-3.5' })}Run</button>
              <button
                type='button'
                onClick={() => {
              ...
              <button type='button' onClick={() => duplicate(workflow)} className='inline-flex items-center gap-1 rounded-md border border-[#2a2a2a] px-2 py-1'>{Icons.duplicate({ className: 'h-3.5 w-3.5' })}Duplicate</button>
              <button type='button' onClick={() => onChange(workflows.filter((item) => item.id !== workflow.id))} className='inline-flex items-center gap-1 rounded-md border border-red-500/40 px-2 py-1 text-red-300'>{Icons.trash({ className: 'h-3.5 w-3.5' })}Delete</button>
            </div>
          </article>
        ))}
      </div>
