...
function subAgentMentionHandle(agent: { instruction: string; sub_id: string }): string {
  const base = subAgentDisplayName(agent)
    .toLowerCase()
    .replace(/[^a-z0-9._-]+/g, '-')
    .replace(/^-+|-+$/g, '')
  const suffix = agent.sub_id.slice(0, 4).toLowerCase()
...
function App() {
  ...
  const handleSend = (instruction: string, selectedMode: SteeringMode, metadata?: Record<string, unknown>) => {
    ...
    const mentionedAgents = subAgents.filter((agent) => mentionMatches.includes(subAgentMentionHandle(agent)))
          ...
          <div className='min-h-0 flex-1'>
            {showSettings || isAdminPath ? (
              ...
              <ChatPanel
                ...
                subAgentMentions={scopedSubAgents.map((agent) => ({ handle: subAgentMentionHandle(agent), label: subAgentDisplayName(agent) }))}
