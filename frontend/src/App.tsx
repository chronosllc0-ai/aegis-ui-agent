...
function App() {
  ...
  const dispatchPromptFromUI = (instruction: string, metadata?: Record<string, unknown>) => {
    handleSend(instruction, mode, metadata)
  }
  ...
  const handleSend = (instruction: string, selectedMode: SteeringMode, metadata?: Record<string, unknown>) => {
    ...
    const latestLogTerminal = latestLog ? ['result', 'error', 'interrupt'].includes(latestLog.type) : false
    const runtimeLooksIdle = !isWorking || latestLogTerminal
    const isNewTask = runtimeLooksIdle
    const action = runtimeLooksIdle ? 'navigate' : 'steer'
                ...
                <div className='min-h-0 min-w-0 flex-1'>
                  {showWorkflow ? (
                    ...
                    <ScreenView
                      ...
                      onExampleClick={(prompt) => dispatchPromptFromUI(prompt, { task_label_source: 'chat', task_label: prompt })}
      ...
      {showSubAgentModal && (
        <SubAgentModal
          ...
          onTryNow={() => {
            localStorage.setItem('aegis_seen_subagent_modal', '1')
            setShowSubAgentModal(false)
            dispatchPromptFromUI('spawn sub-agents: ', { task_label_source: 'chat', task_label: 'spawn sub-agents' })
          }}
        />
      )}
      </main>
