...
async def run_pydantic_adk_navigation(
    *,
    ...
    try:
        ...
        max_tool_calls = int(settings.get("max_tool_calls", 100)) if isinstance(settings, dict) else 100
        model_timeout_seconds = float(settings.get("model_timeout_seconds", 120)) if isinstance(settings, dict) else 120.0
        tool_call_count = 0
        ...
        async def run_tool(tool: str, args_json: str = "{}") -> str:
            ...
            if tool_call_count > max_tool_calls:
                return (
                    "run_tool error: max tool calls exceeded. "
                    "Call fail_task(reason) with a concise reason."
                )
        ...
        if cancel_event is not None:
            run_task = asyncio.create_task(asyncio.wait_for(agent.run(prompt), timeout=model_timeout_seconds))
            ...
            run_result = await asyncio.wait_for(agent.run(prompt), timeout=model_timeout_seconds)
