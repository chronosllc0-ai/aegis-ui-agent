...
class TaskQueueService:
    ...
    @staticmethod
    async def get_task(session: AsyncSession, task_id: str, user_id: str) -> dict[str, Any] | None:
        task = (
    ...
    async def update_progress(session: AsyncSession, task_id: str, progress_pct: int, progress_message: str | None = None) -> None:
        values: dict[str, Any] = {"progress_pct": min(max(progress_pct, 0), 100)}
    ...
    async def mark_running(session: AsyncSession, task_id: str) -> None:
        await session.execute(
            ...
            .values(status="running", started_at=datetime.now(timezone.utc), progress_pct=1)
        )
        await session.commit()
    ...
    async def mark_failed(session: AsyncSession, task_id: str, error: str) -> None:
        task = (await session.execute(select(BackgroundTask).where(BackgroundTask.id == task_id))).scalar_one_or_none()
        if not task:
            return
    ...
    async def pause_task(session: AsyncSession, task_id: str, user_id: str) -> bool:
        result = await session.execute(
            update(BackgroundTask)
            .where(BackgroundTask.id == task_id, BackgroundTask.user_id == user_id, BackgroundTask.status == "queued")
    ...
    async def resume_task(session: AsyncSession, task_id: str, user_id: str) -> bool:
        result = await session.execute(
            update(BackgroundTask)
            .where(BackgroundTask.id == task_id, BackgroundTask.user_id == user_id, BackgroundTask.status == "paused")
    ...
    async def get_active_count(session: AsyncSession) -> int:
        return (
            await session.execute(select(sa_func.count(BackgroundTask.id)).where(BackgroundTask.status == "running"))
        ).scalar() or 0
        ...
        return (
            await session.execute(
                select(sa_func.count(BackgroundTask.id)).where(
                    BackgroundTask.user_id == user_id,
                    ((BackgroundTask.status == "completed") & (BackgroundTask.notification_sent.is_(False)))
    ...
    async def mark_notifications_sent(session: AsyncSession, user_id: str) -> None:
        await session.execute(
            update(BackgroundTask)
            .where(BackgroundTask.user_id == user_id, BackgroundTask.status == "completed")
