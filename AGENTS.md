...
- **Frontend meter wiring (Phase 9):** `_attach_runtime_event_bridge` in `main.py` subscribes the per-user runtime FanOut to every authenticated websocket and forwards every event (`context_meter`, `compaction_checkpoint`, `run_started`, `tool_call_*`, `final_message`, `run_completed`, `error`, …) wrapped as `{kind: "runtime_event", event: {...}}`. The bridge unsubscribes on disconnect and FanOut auto-evicts a callback that raises. `frontend/src/hooks/useContextMeter.ts` seeds from `GET /api/runtime/context-meter/{session_id}` on mount and updates live from the bridged `context_meter` event; `compaction_checkpoint` resets the meter and triggers a one-shot toast. Background-only runs (heartbeat, automation) update the meter the same way as user-driven runs, satisfying hard requirement #13.
- **Web chat from the runtime event stream (Phase 10):** `useWebSocket.ts` renders chat exclusively from `runtime_event` frames (strict `channel === "web"` filter; `context_meter` / `compaction_checkpoint` are exempt). The assistant half is persisted in the WS bridge: `agent_loop.dispatch` carries the inbound chat event's `ws_session_id` through into the `run_started` + `final_message` payloads; the bridge persists with that originating `ws_session_id` and dedupes by `run_id` via the process-global LRU `_RUNTIME_PERSISTED_RUNS` (cap 4096) so a per-user fan-out shared across tabs writes the reply exactly once, attributed to the tab that fired the message. The user half is still persisted at chat-event accept time via `_log_web_message`. `final_message` defensively clears the working-spinner state in case the terminal `run_completed` is dropped, and the frontend folds `status: "error"` from `agent_loop` into the `failed` terminal state.
